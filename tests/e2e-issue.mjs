...
for (const [id, mins] of [["kousho", 180], ["harness", 90], ["rope", 180], ["kogata", 360], ["forklift", 360], ["tailgate", 120], ["toishi", 120], ["teiatsu", 420], ["winch", 240], ["roller", 240], ["chainsaw", 540], ["arc", 600], ["kikaitoishi", 180], ["shovel", 360], ["fuseichi", 360], ["kouatsu", 900], ["ev", 60], ["press", 120], ["youka", 240], ["batsuboku", 360], ["soukou", 360], ["kikaishuzai", 480], ["kanikasen", 480], ["kisokouji", 360], ["kaitai", 420], ["kisokenki", 300], ["kisosousa", 240], ["concrete", 300], ["boring", 300], ["jack", 240], ["kidou", 240], ["robotkyoji", 180], ["robotkensa", 240], ["tire", 240], ["tokushu", 900], ["tamakake", 240], ["crane", 240], ["mobilecrane", 240], ["kensetsulift", 240], ["derrick", 240], ["gondola", 240], ["boiler", 240], ["compressor", 120], ["soukiroom", 120], ["kikoushitsu", 180], ["soukisensui", 120], ["saiatsushitsu", 180], ["josendojo", 90], ["josenshushu", 90], ["josenhaiki", 90], ["josentokutei", 60], ["josentokuteigai", 60], ["tokureikinkyu", 360], ["haikihasai", 120], ["haikishokyaku", 120], ["haikiumetate", 120], ["kakunenkakou", 120], ["kakunensaishori", 120], ["kakunenshiyou", 120], ["kakunengenshiro", 120]]) {
  ...
  if (id === "harness") {
    ...
    check(form.includes("第11条第1項第1号"), "空気圧縮機の様式に持ち場（第1号）の欄がある");
    check(form.includes("吸込みの口") && form.includes("排気ガス"), "空気圧縮機の様式に吸込みの欄がある");
    check(form.includes("下に人がいるあいだは止めない"), "空気圧縮機の様式に、止めないことの欄がある");
    check(form.includes("自動警報装置"), "空気圧縮機の様式に自動警報装置の欄がある");
  } else if (id === "soukiroom") {
    check(form.includes("第11条第1項第2号"), "作業室への送気の様式に持ち場（第2号）の欄がある");
    check(form.includes("減圧表のとおり") && form.includes("早めなかった"), "作業室への送気の様式に減圧表の欄がある");
    check(form.includes("再圧室の用意"), "作業室への送気の様式に再圧室の欄がある");
  } else if (id === "kikoushitsu") {
    check(form.includes("第11条第1項第3号"), "気こう室の様式に持ち場（第3号）の欄がある");
    check(form.includes("耳が抜けない"), "気こう室の様式に加圧の欄がある");
    check(form.includes("いた時間といた圧"), "気こう室の様式に減圧の元になる記録の欄がある");
    check(form.includes("かゆい") && form.includes("圧を戻す"), "気こう室の様式に減圧症の欄がある");
  } else if (id === "soukisensui") {
    check(form.includes("第11条第1項第4号"), "潜水への送気の様式に持ち場（第4号）の欄がある");
  ...
  } else if (id.startsWith("kakunen")) {
    check(form.includes("施設の区分の確認"), `${id}: 核燃料の様式に施設の区分の欄がある`);
    check(form.includes("ハンドフットクロスモニタで手と足を測った"), `${id}: 核燃料の様式に退去の欄がある`);
    check(form.includes("中性子はガンマ線の機械では測れない"), `${id}: 核燃料の様式に中性子の欄がある`);
    check(form.includes("臨界の警報"), `${id}: 核燃料の様式に臨界の警報の欄がある`);
    check(form.includes("鳴ったら、まず逃げる。止めに戻らない"), `${id}: 核燃料の様式に「まず逃げる」の欄がある`);
    if (id === "kakunengenshiro") {
      check(form.includes("使用済燃料を水から出さなかった"), "原子炉施設の様式に、水から出さないの欄がある");
    } else {
      check(form.includes("分解の前に、中に何がどれだけあるかを確かめた"), `${id}: 加工施設等の様式に、分解の前の確認の欄がある`);
    }
    ...
    check(form.includes("空気呼吸器の残圧と警報"), "特例緊急作業の様式に空気呼吸器の欄がある");
    check(form.includes("目的を果たしたら、それ以上やらなかった"), "特例緊急作業の様式に「戻る」の欄がある");
    check(form.includes("人形で全員が胸骨圧迫をした"), "特例緊急作業の様式に応急手当の欄がある");
    ...
    check(form.includes("人形で全員が胸骨圧迫をした"), "再圧室の様式にそ生法の欄がある");
    check(form.includes("再圧室の中では使わない"), "再圧室の様式に AED の欄がある");
  } else if (id === "boiler") {
    check(form.includes("水面計") && form.includes("水位を自分の目で見た"), "小型ボイラーの様式に水面計の欄がある");
    check(form.includes("水を入れない") && form.includes("自然に冷ます"), "小型ボイラーの様式に、水が見えなくなったときの欄がある");
    check(form.includes("プレパージ") && form.includes("点火し直しを繰り返さなかった"), "小型ボイラーの様式に点火の欄がある");
    check(form.includes("吹出し") && form.includes("ポストパージ"), "小型ボイラーの様式に吹出しと停止の欄がある");
    check(form.includes("一酸化炭素"), "小型ボイラーの様式に換気の欄がある");
    check(form.includes("ボイラー技士免許"), "小型ボイラーの様式に、線を超えたら免許が要る欄がある");
    check(!form.includes("電鈴"), "小型ボイラーの様式に電鈴の合図が無い");
    check(!form.includes("作業床の高さ"), "小型ボイラーの様式に「作業床の高さ」が無い");
  } else if (id === "gondola") {
