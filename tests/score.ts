...
/* 分類が同じでも、言われた中身が違えば別に出す */
const mixed = summarize([
  { tag: "手順の飛ばし", message: "先に離れを見ろ。", why: "あとで柱ごと動かすことになる。" },
  { tag: "手順の飛ばし", message: "水平が先だ。", why: "受け材が乗ってから水平を出すと沈む。" },
  { tag: "手順の飛ばし", message: "先に離れを見ろ。", why: "あとで柱ごと動かすことになる。" },
]);
check(mixed.length === 2, "同じ分類でも中身が違えば別の指摘として出す");
check(mixed[0].n === 2 && mixed[1].n === 1, "それぞれの回数が付く");
