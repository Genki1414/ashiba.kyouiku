...
`/setup` は環境変数を2段に分けて表示します。

| 表示 | 意味 |
|---|---|
| **サーバ側（実行時に読まれる）** | 実行中の関数から見えているか |
| **ブラウザ側（ビルド時に埋め込まれる）** | ビルド時に値が渡っていたか |

- 両方とも未設定 → **そのデプロイは変数を追加する前に作られている**。Redeploy が要る
  （Vercel は環境変数をデプロイ作成時に焼き込むので、あとから追加しても既存のデプロイには届かない）
- サーバ側だけ設定済み → **ビルドに値が渡っていない**。
  `NEXT_PUBLIC_` の変数から Sensitive を外し、ビルドキャッシュを使わずに Redeploy
...
`supabase/apply-all.sql` を SQL Editor でもう一度実行してください。
`/setup` の「スキーマの版」が **0011** になれば済みです。

特別教育を何種類でも増やせるようにしました。詳しくは docs/13。

- `courses`（講座）を足し、`lessons` / `enrollments` / `orders` が
  どの講座のものかを持つようになりました
- **単元IDが `1-1` から `ashiba:1-1` に変わります。**
  視聴記録と照合の記録も、同じ流しの中で付け替わります。
  受講中の人がいても、記録は消えません
- 受講コード（席）は1講座ぶんになりました。いまある席は足場のものとして扱われます

**このファイルから開発用の種データを外しました。**
これまでは流すたびに「開発テスト工業」「DEV-0001」「0円の注文」が
戻ってきていました。0011 を流したあとに `/owner` を見て、
まだ0円の注文が残っていたら `supabase/cleanup-dev-seed.sql` を流してください。
...
`supabase/apply-all.sql` を SQL Editor でもう一度実行してください。
`/setup` の「スキーマの版」が **0012** になれば済みです。

人は辞めますし、よその会社へ移ります。
それでも受けた記録は、受けた当時の会社に残らないと困ります
（教育を行った事業者が3年保存する決まり）。詳しくは docs/14。

- `memberships`（在籍）を足しました。申し込んだ日・許可した日・抜けた日を持ちます
- **いま所属している人の在籍は、そのまま起こします**（許可済みとして）
- `enrollments.company_id`（どの会社の席で受けたか）を足し、
  いまある受講は席の注文元、無ければいまの所属で埋めます
...
`supabase/apply-all.sql` を SQL Editor でもう一度実行してください。
`/setup` の「スキーマの版」が **0013** になれば済みです。

**特別教育を行っているのは、この仕組みの運営（東北三上機材株式会社）です。**
修了証もその名義で出しているので、記録を保存する義務はこちらにあります。

0012 までは、受講コードの引き換えを取り消したときに
視聴記録・試験・実務・照合ログを消していました。
消すのをやめて、その受講を **閉じる**（`enrollments.closed_at`）形にします。

- 取り消した受講は閉じるだけ。記録は残る
- 同じ人がもう一度受けると、新しい受講が0から始まる
- 閉じた受講は名簿に出ないが、データベースには残る
