...
/** 既定の単価（税抜・円）。
    ...
    ── よその値段（2026年8月に調べた。オンライン受講・税込） ──
      ...
      ボーリングマシン（学科7時間＋実技5時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/58）。
        → ここ 7,000円（税抜）＝ 7,700円（税込）
        学科7時間。小型車両系の三つ・基礎工事用の運転・コンクリート打設用と同じ長さ。
        同じ 7,000円に置いた。
        実技の手引きと実施記録の様式が付く（/edu/boring/drill）。

      ジャッキ式つり上げ機械（学科6時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/59）。
        → ここ 6,000円（税抜）＝ 6,600円（税込）
        学科6時間。フォークリフト・ローラー・機械集材装置と同じ長さなので、同じ 6,000円に置いた。
        実技の手引きと実施記録の様式が付く（/edu/jack/drill）。

      軌道装置の動力車（学科6時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/60）。
        → ここ 6,000円（税抜）＝ 6,600円（税込）
        学科6時間。ジャッキ式つり上げ機械・簡易架線集材装置等と同じ長さなので、同じ 6,000円に置いた。
        実技の手引きと実施記録の様式が付く（/edu/kidou/drill）。

      産業用ロボット（教示等）（学科7時間＋実技3時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/61）。
        → ここ 7,000円（税抜）＝ 7,700円（税込）
      ...
      自動車用タイヤの空気充てん（学科5時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/63）。
        → ここ 5,500円（税抜）＝ 6,050円（税込）
      ...
      デリック（5トン未満）（学科9時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/77）。
        → ここ 8,000円（税抜）＝ 8,800円（税込）
        学科9時間。クレーン・移動式クレーンと同じ長さなので、同じ 8,000円。
        実技の手引きと実施記録の様式が付く（/edu/derrick/drill）。

      四アルキル鉛等業務（**学科6時間だけ。実技は無い**）
        **よその値段はまだ調べていない**（docs/80）。
        → ここ 6,000円（税抜）＝ 6,600円（税込）
        学科6時間。**実技が無いので、手引きも実施記録も付かない。**
        酸欠・硫化水素（学科5時間30分）の一つ上に置いた。
      ...
      ゴンドラ（学科5時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/78）。
        → ここ 5,500円（税抜）＝ 6,050円（税込）
        学科5時間。建設用リフトと同じ長さで、科目の形もそっくりなので、同じ 5,500円。
        実技の手引きと実施記録の様式が付く（/edu/gondola/drill）。

      建設用リフト（学科5時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/76）。
        → ここ 5,500円（税抜）＝ 6,050円（税込）
        学科5時間。玉掛け・自動車用タイヤと同じ長さなので、同じ 5,500円。
        実技の手引きと実施記録の様式が付く（/edu/kensetsulift/drill）。
      ...
      クレーン（5トン未満）・跨線テルハ（学科9時間＋実技4時間。ここで出すのは学科だけ）
        **よその値段はまだ調べていない**（docs/67）。
        → ここ 8,000円（税抜）＝ 8,800円（税込）
        学科9時間。チェーンソー・産業用ロボット（検査等）と同じ長さなので、同じ 8,000円に置いた。
        **目録の2行（クレーン5トン未満・跨線テルハ5トン以上）を、この1本で埋めている。**
        実技の手引きと実施記録の様式が付く（/edu/crane/drill）。
