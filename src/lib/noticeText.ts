...
export type NoticeKind =
  ...
  | "issue_ng"    // 発行申請を断った
  | "opened"      // 個人の申込みの入金を確認して、講座を開けた（0039）
  /* 会社の担当者から、受講する人へ */
...
const DEFS: Record<NoticeKind, Def> = {
  ...
  /* 担当者が席を直接配った（0028・0031）。受講者はコードを打たない。
     **押すと、その講座がそのまま開く。**「コードを使う」を押させる
     案もあったが、押し忘れで止まるだけなので、開く方にした
     （げんきさん 2026-09-09） */
  given: {
    t: "受講コードが届きました",
  ...
  /* ひとりで受ける（0039）。本人が申し込み、入金を確認した瞬間に
     その人の席が立つ。コードを配る・打つ、という手順が無いので、
     「受講コードが届きました」ではなく「受講できます」と言う。
     押すと、その講座がそのまま開く */
  opened: {
    t: "受講できるようになりました",
    d: "お振込みを確認しました。押すと、その講座が開きます",
    from: "owner",
    href: (c) => (c ? `/edu/${c}` : "/edu"),
    needsCourse: true,
  },
