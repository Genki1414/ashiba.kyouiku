...
export function learnerSteps(me: Who): Step[] {
  const steps: Omit<Step, "state">[] = [
    ...
    {
      t: "会社とつなぐ",
      d: "自分の会社を検索して申し込みます。つながっていないと名簿に登録されず、修了証も発行できません",
      href: "/join",
    },
    {
      t: "会社の承認を待つ",
      d: "教育担当者が承認すると、名簿に登録されます。お急ぎの場合は担当者にご連絡ください",
      href: "/join",
    },
    {
      t: "受講の準備",
      d: "マイページで**氏名と生年月日**を入れ、カメラの使用に同意して顔を1枚登録します。氏名は修了証に載るもので、**入れるのは一度だけ**です",
    },
    {
      /* 受け直しを防ぐ。前の会社や教習機関で取ったものを先に入れておくと、
         担当者が受講コードを無駄に配らずに済む（げんきさん 2026-09-09） */
      t: "取得済みの資格を登録する",
      d: "前職や教習機関で取った資格を、マイページから登録します。**同じ特別教育を受け直す必要はありません。**登録すると講座一覧に「取得済」と出ます",
      href: "/me",
    },
...
export function adminSteps(me: Who): Step[] {
  const steps: Omit<Step, "state">[] = [
    ...
    {
      t: "受講コードを申し込む",
      d: "受ける人数分を申し込みます。お振込みの確認後に発行されます",
      href: "/admin",
    },
    {
      t: "参加コードを配る",
      d: "受ける方に登録してもらい、8文字の参加コードで自社に申し込んでもらいます",
      href: "/admin",
    },
    {
      t: "申し込みを承認する",
      d: "**ここを忘れると教材が開きません。**申し込んだだけの方は名簿に登録されません",
      href: "/admin",
    },
    {
      /* 配る前に確かめる所。取得済みの人に配ると、受講コードが1枚無駄になる */
      t: "取得済みの資格を確かめる",
      d: "受ける人がマイページに登録した資格を、名簿で確かめます。**取得済みの人には受講コードを配れません**（同じ特別教育を受け直す必要はありません）",
      href: "/admin",
    },
