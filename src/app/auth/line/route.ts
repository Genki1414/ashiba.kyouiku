...
export async function GET(req: NextRequest) {
  ...
  /* すでに結んである人なら、**表示名だけ入れ直す。**
     0037 で表示名を持つようにしたが、既に結んである人は
     ここを素通りしていたので、いつまでも空のままだった
     （げんきさん 2026-09-10「どのLINEと繋がってるのか分からない」）。
     相手が LINE の名前を変えることもあるので、通るたびに新しくする。
     失敗しても止めない。名前が出ないだけで、ログインはできる */
  if (found) await linkLine(found.id, who.sub, who.name);
