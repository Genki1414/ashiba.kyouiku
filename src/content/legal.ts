...
export function seller() {
  return {
    ...
    /** 適格請求書発行事業者の登録番号（T＋13桁）。
        ...
        **既定値をここに直接書いている。**理由は上のとおりで、
        国税庁が公表している番号であり、請求書に刷って送るもの。隠す物ではない
        （会社名・代表者・住所・電話・振込先と同じ扱い）。

        Vercel の環境変数に入れると Sensitive で読み出せなくなり、
        店を増やしたときに片方が「未設定」のままになる。実際になった
        （2026-09-07、特別教育ドットコムの特商法が「未設定」で出ていた）。 */
    invoiceNo: tidyInvoice(get("SELLER_INVOICE_NO", "T4370001041531")),
    /* 振込先。請求書に載せる。
...
export function tokushoho(prices: CoursePrice[]): Item[] {
  ...
  return [
    ...
    {
      /* ── 支払時期（げんきさん 2026-09-11）──
           「支払い期限は請求書発行から1週間後。
             支払い確認が取れなければ受講不可」

         前は「支払期限は定めていません」と書いていた。ところが
         申込みの画面には期限の日付が出ていて、**同じ取引について
         画面ごとに違うことを言っていた。**特商法の必須記載なので、
...
/** 実務トレーニングの記録。**足場屋革命だけ。**
    特別教育ドットコムでは実務トレーニングを売っていないので、
    「預かるもの」に並べると、預かっていない物を並べることになる（2026-09-07）。
    出すかどうかは画面側（src/app/legal/privacy/page.tsx）が BRAND で決める */
export const TRAINING_DATA: { k: string; v: string } = {
  k: "実務トレーニングの記録",
  v: "章ごとの点数・所要時間と、指摘された内容",
};
